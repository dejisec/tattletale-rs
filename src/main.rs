...
fn verify_inputs(args: &Args) -> Result<()> {
    ...
    for p in &args.ditfiles {
        if !p.exists() { bail!("DIT file not found: {}", p.display()); }
    }
    for p in &args.potfiles {
        if !p.exists() { warn!("potfile not found: {} (continuing)", p.display()); }
    }
    for p in &args.targetfiles {
        if !p.exists() { warn!("target file not found: {} (continuing)", p.display()); }
    }
    Ok(())
}
...
fn main() {
    ...
    let potfiles_existing: Vec<PathBuf> = args.potfiles.iter().filter(|p| p.exists()).cloned().collect();
    let targetfiles_existing: Vec<PathBuf> = args.targetfiles.iter().filter(|p| p.exists()).cloned().collect();
    if let Err(e) = engine.load_from_file_paths_with_threshold(&args.ditfiles, &potfiles_existing, &targetfiles_existing, threshold) {
