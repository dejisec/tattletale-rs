...
pub fn analyze_creds(creds: &[Credential]) -> BasicStats {
	...
	for c in creds {
		...
		if c.is_cracked { passwords.push(c.cleartext.clone()); }
	}
	let all = hashes.len();
...
pub fn calculate_statistics(all: &[Credential]) -> Statistics {
	...
	for c in all {
		if c.is_user_account {
			user.push(c.clone());
			if !c.is_hash_null && !c.domain.is_empty() { valid_domain_user.push(c.clone()); }
		}
		if c.is_machine_account {
			machine.push(c.clone());
			if !c.is_hash_null { valid_machine.push(c.clone()); }
		}
		if !c.is_hash_null {
			if c.is_hash_type_lm { lm.push(c.clone()); }
			if c.is_hash_type_nt { nt.push(c.clone()); }
			if c.is_hash_type_both { both.push(c.clone()); }
		} else {
			null.push(c.clone());
		}
		if c.domain.is_empty() { no_domain.push(c.clone()); }
	}
